...
class VanillaDecoder(nn.Module):
    ...
    def __init__(self, hidden_size, output_size, max_length,
        ...
        self.log_softmax = nn.LogSoftmax(
            dim=-1)  # work with NLLLoss = CrossEntropyLoss
    ...
    def forward(self, context_vector, targets):
        ...
        decoder_outputs = Variable(
            torch.zeros(
                ...
                self.output_size))  # (time_steps, batch_size, vocab_size)
        ...
        for t in range(max_target_length):
            decoder_outputs_on_t, decoder_hidden = self.forward_step(
    ...
    def evaluate(self, context_vector):
        ...
        for t in range(self.max_length):
            decoder_outputs_on_t, decoder_hidden = self.forward_step(
                decoder_input, decoder_hidden)
            decoder_outputs[t] = decoder_outputs_on_t
            decoder_input = self._decode_to_index(
                decoder_outputs_on_t)  # select the former output as input
